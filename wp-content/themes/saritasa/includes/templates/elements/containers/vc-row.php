...
//        $parallax_class = 'standard_section';
    ...
    echo '<div id="' . $row_id . '" ' . $fullscreen_anchor_id . ' ' . $row_percent_padding_attr . ' class="vc_row vc_row-fluid ' . trim($top_level_class . $main_class . $disable_class . $equal_height_class . $parallax_class . ' ' . $vertically_center_class . ' ' . $class) . '" ' . $using_custom_text_color . ' style="' . $style . '">';
//    echo '
//		<div id="' . $row_id . '" ' . $fullscreen_anchor_id . ' data-midnight="' . strtolower($midnight_color) . '" ' . $row_percent_padding_attr . ' data-bg-mobile-hidden="' . $background_image_mobile_hidden . '" class="wpb_row vc_row-fluid vc_row ' . $top_level_class . $main_class . $disable_class . $equal_height_class . $parallax_class . ' ' . $vertically_center_class . ' ' . $class . ' " ' . $using_custom_text_color . ' style="' . $style . '">';
    ...
    if ($video_bg) {
        ...
        if (floatval(get_bloginfo('version')) >= "3.6") {
            ...
            wp_register_script('wp-mediaelement', get_template_directory_uri() . '/js/mediaelement-and-player.min.js', ['jquery'], '1.0', true);
        ...
        if (!empty($video_external) && vc_extract_youtube_id($video_external)) {
            ...
            if (!empty($video_webm)) {
                $video_markup .= '<source src="' . $video_webm . '" type="video/webm">';
            }
            if (!empty($video_mp4)) {
                $video_markup .= '<source src="' . $video_mp4 . '"  type="video/mp4">';
            }
            if (!empty($video_ogv)) {
                $video_markup .= '<source src="' . $video_ogv . '" type="video/ogg">';
            }
    ...
    if ($enable_shape_divider == 'true') {
        ...
        $shape_divider_pos = ($shape_divider_position == 'both') ? ['top', 'bottom'] : [$shape_divider_position];
    ...
    echo $extra_container_div . '<div class="col span_12">' . do_shortcode($content) . '</div></div>' . $extra_container_div_closing;
