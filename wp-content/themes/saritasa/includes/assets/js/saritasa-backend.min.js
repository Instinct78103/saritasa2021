...
window.vc || (window.vc = {}), function () {
  ...
  vc.templateOptions = {
    ...
    custom: {evaluate: /<#([\s\S]+?)#>/g, interpolate: /\{\{\{([\s\S]+?)\}\}\}/g, escape: /\{\{([^\}]+?)\}\}(?!\})/g},
  };
  ...
  _.isUndefined(window.vc) && (window.vc = {}), window.Vc_postSettingsEditor = Backbone.View.extend({
    $editor: !1, sel: 'wpb_csseditor', ace_enabled: !1, initialize: function (sel) {
  ...
  _.isUndefined(window.vc) && (window.vc = {}), vc.showSpinner = function () {
    ...
    buildFiltering: function () {
      ...
      var mapped = vc.getMapped(tag), as_parent = !(!tag || _.isUndefined(mapped.as_parent)) && mapped.as_parent;
    ...
    createElement: function (e) {
      ...
      if (!this.preventDoubleExecution) {
        this.preventDoubleExecution = !0, this.do_render = !0, e.preventDefault(), tag = $(e.currentTarget).data('tag'), row_params = {}, row_inner_params = {}, !(column_params = {width: '1/1'}) === this.model && 'vc_row' !== tag ? (this.builder.create({
          shortcode: 'vc_row',
          params: row_params,
        }).create({
          shortcode: 'vc_column',
        ...
        'vc_row' === tag ? this.builder.create({
          ...
          parent_id: this.builder.lastID(),
          params: column_params,
        }) : 'vc_row_inner' === tag && (column_params = {width: '1/1'}, this.builder.create({
    ...
    filterElements: function (e) {
      ...
      var $control = $(e.currentTarget), filter = '[data-vc-ui-element="add-element-button"]',
    ...
    render: function (model, prepend) {
      ...
      if (!this.preventDoubleExecution) {
        ...
        row_params = {}, model = !(column_params = {width: '1/1'}) === this.model ? (row = vc.shortcodes.create({
          shortcode: 'vc_row',
          params: row_params,
          ...
          parent_id: column.id,
          root_id: row.id,
        }) : row) : 'vc_row' === tag ? (row_inner_params = {}, column_inner_params = {width: '1/1'}, row = vc.shortcodes.create({
    ...
    fixElContainment: function () {
      ...
      var containment = [20 - this.$el.width(), 0, this.$body.width() - 20, this.$body.height() - 30],
        positions = this.$el.position(), new_positions = {};
    ...
    events: {
      ...
      'mouseover [data-transparent=panel]': 'addOpacity',
      'mouseout [data-transparent=panel]': 'removeOpacity',
    }, saved_css_data: '', saved_title: '', $title: !1, editor: !1, post_settings_editor: !1, initialize: function () {
    ...
    events: {
      ...
      var $button = $(e.currentTarget), template_name = $button.closest('[data-vc-ui-element="template-title"]').text();
    ...
    removeTemplate: function (e) {
      ...
      if (confirm(window.i18nLocale.confirm_deleting_template.replace('{template_name}', template_name))) {
        var template_id = $template.data('template_id'), template_type = $template.data('template_type'),
    ...
    buildTemplatePreview: function (e) {
      e && e.preventDefault && e.preventDefault();
      try {
        var url, $el = $(e.currentTarget), $wrapper = $el.closest('[data-template_id]');
        if ($wrapper.hasClass('vc_active') || $wrapper.hasClass('vc_loading')) $el.vcAccordion('collapseTemplate'); else {
          ...
          $el.vcAccordion('collapseTemplate', function () {
            var templateId = $wrapper.data('template_id'), templateType = $wrapper.data('template_type');
            if (templateId && !localContentChilds) {
              var question = '?';
              -1 < window.ajaxurl.indexOf('?') && (question = '&'), url = window.ajaxurl + question + $.param({
    ...
    builder: function () {
      ...
      e && e.preventDefault && e.preventDefault();
      var layout = $(e.currentTarget).attr('data-cells'), columns = this.model.view.convertRowColumns(layout);
      this.$input.val(columns.join(' + '));
    },
  ...
  (TabsLine = function (element, options) {
    ...
    return this.each(function () {
      var $this = $(this), optionsData = $this.data('vcUiTabsLine'), data = $this.data('vc.tabsLine'),
  ...
  window.vc.HelperPrompts = {
    uiEvents: {render: 'removeAllPrompts'}, removeAllPrompts: function () {
  ...
  window.vc.HelperPanelViewHeaderFooter = {
    ...
    toggleOpacity: function (e) {
      e.preventDefault();
      var styles, hasStyle, _this = this, $target = this.$el, $panel = $target.find($target.data('vcPanel')),
        $panelContainer = $panel.closest($panel.data('vcPanelContainer')), $trigger = $(e.currentTarget);
      void 0 === $target.data('vcHasHeight') && $target.data('vcHasHeight', (styles = $target.attr('style'), hasStyle = !1, styles && styles.split(';').forEach(function (e) {
      ...
      }), hasStyle)), $target.hasClass('vc_minimized') ? (void 0 === $target.data('vcMinimizeHeight') && $target.data('vcMinimizeHeight', $(window).height() - .2 * $(window).height()), $target.animate({height: $target.data('vcMinimizeHeight')}, {
        duration: 400,
        start: function () {
          $trigger.prop('disabled', !0), $target.addClass('vc_animating'), _this.tabsMenu && _this.tabsMenu() && _this.tabsMenu().vcTabsLine('moveTabs');
        },
        complete: function () {
          $target.removeClass('vc_minimized'), $target.removeClass('vc_animating'), $target.data('vcHasHeight') || $target.css({height: ''}), _this.trigger('afterUnminimize'), $trigger.prop('disabled', !1);
        },
        ...
        complete: function () {
          $target.addClass('vc_minimized'), $target.removeClass('vc_animating'), $target.css({height: ''}), _this.trigger('afterMinimize'), $trigger.prop('disabled', !1);
        },
      }));
    },
  ...
  window.vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
    panelName: 'template_window', showMessageDisabled: !1, initialize: function () {
    ...
    panelName: 'template_window', showMessageDisabled: !1, show: function () {
    ...
    if ($this = this.$element, i = 0, ($triggers = this.getContainer().find('[data-vc-preview-handler]').each(function () {
      ...
      return $this[0] !== this;
    })).length && $.fn.vcAccordion.call($triggers, 'hide'), this.isActive()) $.fn.vcAccordion.call($this, 'hide'); else {
      $.fn.vcAccordion.call($this, 'show');
      var $triggerPanel = $this.closest('.vc_ui-list-bar-item'), $wrapper = $this.closest('[data-template_id]'),
  ...
  window.vc.element_start_index = 0, window.vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
    ...
    createElement: function (e) {
      var options, model, column, row, showSettings, row_params, inner_row_params, column_params, inner_column_params,
        tag, $control, preset, presetType, closestPreset;
      if (e && e.preventDefault && e.preventDefault(), tag = ($control = $(e.currentTarget)).data('tag'), row_params = {}, column_params = {width: '1/1'}, (closestPreset = $control.closest('[data-preset]')) && (preset = closestPreset.data('preset'), presetType = closestPreset.data('element')), !1 === this.model) if (window.vc.storage.lock(), 'vc_section' === tag) {
        var modelOptions = {shortcode: tag};
        preset && 'vc_section' === presetType && (modelOptions.preset = preset), model = vc.shortcodes.create(modelOptions);
      } else {
        var rowOptions = {shortcode: 'vc_row', params: row_params};
        ...
        parent_id: row.id,
        root_id: row.id,
      })) : (inner_row_params = {}, inner_column_params = {width: '1/1'}, window.vc.storage.lock(), row = vc.shortcodes.create({
        shortcode: 'vc_row_inner',
    ...
    events: {
      ...
      var options, showSettings, newData, i, column_params, row_params, inner_row_params, inner_column_params, $control,
        tag, preset, presetType, closestPreset;
      if (e && e.preventDefault && e.preventDefault(), tag = ($control = $(e.currentTarget)).data('tag'), row_params = {}, column_params = {width: '1/1'}, (closestPreset = $control.closest('[data-preset]')) && (preset = closestPreset.data('preset'), presetType = closestPreset.data('element')), this.prepend && (window.vc.activity = 'prepend'), 0 == this.model) if ('vc_section' === tag) {
        var modelOptions = {shortcode: tag};
        ...
        var columnOptions = {shortcode: 'vc_column', parent_id: this.builder.lastID(), params: column_params};
        ...
        params: column_params,
        parent_id: this.builder.lastID(),
      }) : (inner_row_params = {}, inner_column_params = {width: '1/1'}, this.builder.create({
        shortcode: 'vc_row_inner',
        params: inner_row_params,
        parent_id: this.model.id,
        order: this.prepend ? this.getFirstPositionIndex() : vc.shortcodes.nextOrder(),
  ...
  window.vc.ExtendPresets = {
    ...
    uiEvents: {init: 'addEvents', render: 'hideDropdown', afterRender: 'afterRenderActions'},
    ...
    showDropdownMenu: function () {
      var tag = this.model.get('shortcode'), $this = $(this);
      $this.data('vcSettingsMenuLoaded') && tag === $this.data('vcShortcodeName') || this.reloadSettingsMenuContent();
    },
    addEvents: function () {
      var $tab = this.$el.find('.vc_edit-form-tab.vc_active'), tag = this.model.get('shortcode'), _this = this;
    ...
    saveSettings: function (title, is_default) {
      var shortcode_name = this.model.get('shortcode'), data = JSON.stringify(this.getParamsForSettingsPreset());
    ...
    fetchSaveSettingsDialogAjaxData: function () {
      return {action: 'vc_action_render_settings_preset_title_prompt', vc_inline: !0, _vcnonce: window.vcAdminNonce};
    },
    ...
    showSaveSettingsDialog: function (is_default) {
      ...
      this.isSettingsPresetDefault = !!is_default, this.fetchSaveSettingsDialog(function (created) {
        var $contentContainer = _this.$el.find('.vc_ui-panel-content-container'),
          $prompt = $contentContainer.find('.vc_ui-prompt-presets'), $title = $prompt.find('.textfield');
        ...
        if ('undefined' !== window.vc_vendor_settings_presets[_this.model.get('shortcode')] ? $viewPresetsButton.removeAttr('disabled') : $viewPresetsButton.attr('disabled', 'disabled'), $prompt.addClass('vc_visible'), $title.focus(), $contentContainer.addClass('vc_ui-content-hidden'), created) {
          var $btn = $prompt.find('#vc_ui-save-preset-btn'), delay = 0;
    ...
    saveAsDefaultSettings: function (id, doneCallback) {
      var shortcode_name = this.model.get('shortcode'), presetId = id || this.settingsPresetId;
    ...
    setSettingsMenuContent: function (html) {
      var $button = this.$el.find(this.settingsButtonSelector), $menu = this.$el.find(this.settingsMenuSelector),
        shortcode_name = this.model.get('shortcode'), _this = this;
    ...
    reloadSettingsMenuContent: function () {
      var shortcode_name = this.model.get('shortcode'), $button = this.$el.find(this.settingsButtonSelector),
        success = !1;
  ...
  window.vc.ExtendTemplates = {
    fetchSaveTemplateDialogAjaxData: function () {
      return {action: 'vc_action_render_settings_templates_prompt', vc_inline: !0, _vcnonce: window.vcAdminNonce};
      ...
      this.fetchSaveTemplateDialog(function (created) {
        var $tab = _this.$el.find('.vc_ui-panel-content-container'), $prompt = $tab.find('.vc_ui-prompt-templates'),
        ...
        if ($tab.find('.vc_ui-prompt.vc_visible').removeClass('vc_visible'), $prompt.addClass('vc_visible'), $title.focus(), $tab.addClass('vc_ui-content-hidden'), created) {
          var delay = 0, $btn = $prompt.find('#vc_ui-save-templates-btn');
          $prompt.on('submit', function () {
            var title = $title.val();
  ...
  window.vc.TemplateLibraryView = vc.PanelView.vcExtendUI(vc.HelperAjax).extend({
    ...
    getLibrary: function () {
      if (this.loaded) this.showLibrary(); else {
        this.checkAjax();
        var data = this.getStorage('templates'), _this = this;
    ...
    getStorage: function (key) {
      var now = Date.now(), expiresIn = localStorage.getItem('vc4-' + key + '_expiresIn');
    ...
    addTemplateStatus: function (template) {
      var statusHtml = '', myTemplate = _.find(this.myTemplates, {id: template.id});
      if (myTemplate) {
    ...
    loadMyTemplates: function () {
      ...
      this.myTemplates.forEach(function (item) {
        renderedOutput += _this.compiledTemplate({post_id: item.post_id, title: item.title});
    ...
    previewButton: function (e) {
      var $template = $(e.currentTarget), imgUrl = $template.data('preview-url'), title = $template.data('title'),
        templateId = $template.data('template-id'), templateVersion = $template.data('template-version');
    ...
    downloadTemplate: function (id) {
      ...
      this.ajax = $.ajax({
        type: 'POST',
        url: window.ajaxurl,
        data: {action: 'vc_shared_templates_download', id: id, _vcnonce: window.vcAdminNonce},
        dataType: 'json',
        context: this,
      }).done(function (response) {
    ...
    window.vcTemplatesLibraryData && (window.vc.templatesLibrary = new vc.TemplateLibraryView({el: '[data-vc-ui-element="panel-edit-element-tab"][data-tab="shared_templates"]'}), window.vc.templatesLibrary.myTemplates = window.vcTemplatesLibraryData.templates || [], window.vc.templatesLibrary.loadMyTemplates());
  });
}(window.jQuery), function ($) {
  'use strict';
  ...
  window.vc.PresetSettingsUIPanelFrontendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperAjax).vcExtendUI({
    ...
    createPreset: function (e) {
      ...
      _.isUndefined(vc.ShortcodesBuilder) || (this.builder = new vc.ShortcodesBuilder), preset = ($control = $(e.currentTarget)).data('preset'), tag = $control.data('tag'), column_params = {width: '1/1'}, rowOptions = {
        shortcode: 'vc_row',
        params: {},
    ...
    removePreset: function (e) {
      ...
      var closestPreset = jQuery(e.currentTarget).closest('[data-vc-ui-delete="preset-title"]'),
        presetId = closestPreset.data('preset'), presetParent = closestPreset.data('preset-parent');
  ...
  window.vc_toTitleCase = function (str) {
    ...
    for (i in columns_count = (columns = cells.split('_')).length, numbers_sum = 0, columns) if (!isNaN(parseFloat(columns[i])) && isFinite(columns[i])) {
    ...
    var blocklist = 'blockquote|ul|ol|li|dl|dt|dd|table|thead|tbody|tfoot|tr|th|td|h[1-6]|fieldset|figure',
      blocklist1 = blocklist + '|div|p', blocklist2 = blocklist + '|pre', preserve_linebreaks = !1, preserve_br = !1,
      preserve = [];
    ...
    var preserve_linebreaks = !1, preserve_br = !1,
    ...
    var params = model.get('params'), preset = model.get('preset');
    params = _.extend({}, vc.getDefaults(model.get('shortcode')), params), preset && window.vc_all_presets[preset] && (params = window.vc_all_presets[preset], void 0 !== vc.frame_window && window.vc_all_presets[preset].css && vc.frame_window.vc_iframe.setCustomShortcodeCss(window.vc_all_presets[preset].css)), model.set({params: params}, {silent: !0});
    ...
    if ('new' !== action && 'clone' !== action || $elem.find('.vc_control.column_toggle').click(), 'new' === action) {
      ...
      for (exclude = ['white', 'black'], $options = ($select = $elem.find('[name=values_color]')).find('option'), i = 0; !(100 < i++);) if (random = Math.floor(Math.random() * $options.length), -1 === window.jQuery.inArray($options.eq(random).val(), exclude)) {
...
}, _.isUndefined(window.vc) && (window.vc = {}), window.vc.filters = {templates: []}, window.vc.addTemplateFilter = function (callback) {
  ...
  window.init_textarea_html = function ($element) {
    ...
    if (hex.length < 6) for (var i = 6 - hex.length - 1; 0 <= i; i--) hex = '0' + hex;
    return '#' + hex;
    ...
    constructor: Suggester, init: function () {
      ...
      e.preventDefault();
      var $label = $(e.currentTarget), index = parseInt($label.data('index'), 10);
  ...
  var VcLoopEditorView = Backbone.View.extend({
    ...
    updateCheckbox: function (e) {
      var input_name = $(e.currentTarget).data('input'), $input = $('[data-name=' + input_name + ']', this.$el),
        value = [];
    ...
    events: {
      ...
      return _.each(this.settings, function (field) {
        ...
        if ($field.is('script')) {
          ...
          html += template(_.extend({}, {name: '', label: '', value: [], options: '', description: ''}, field));
        }
  ...
  var GoogleFonts = Backbone.View.extend({
    ...
    fontStyleDropdownRender: function () {
      var str_arr = this.font_types.split(','), oel = '',
  ...
  vc.edit_form_callbacks = [], vc.atts = {
    ...
    clone: function (clonedModel, paramValue, paramSettings) {
      ...
      var shortcodeParams;
      !_.isEmpty(paramValue) || _.isUndefined(paramSettings) || _.isUndefined(paramSettings.settings) || _.isUndefined(paramSettings.settings.auto_generate) || 1 != paramSettings.settings.auto_generate || ((shortcodeParams = shortcodeModel.get('params'))[paramSettings.param_name] = Date.now() + '-' + vc_guid(), shortcodeModel.set({params: shortcodeParams}, {silent: !0}));
    },
    ...
    init: function (param, $field) {
      $('.vc_color-control', $field).each(function () {
        var $alpha, $alpha_output, $pickerContainer, $control = $(this), value = $control.val().replace(/\s+/g, ''),
          alpha_val = 100;
        value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/) && (alpha_val = 100 * parseFloat(value.match(/rgba\(\d+\,\d+\,\d+\,([^\)]+)\)/)[1])), $control.wpColorPicker({
          ...
          alpha_val = parseFloat($alpha.val()), iris = $control.data('a8c-iris'), color_picker = $control.data('wp-wpColorPicker'), $alpha_output.val($alpha.val() + '%'), iris._color._alpha = alpha_val / 100, $control.val(iris._color.toString()), color_picker.toggler.css({backgroundColor: $control.val()});
    ...
    init: function (param, $field) {
      $('.vc_link-build', $field).on('click', function (e) {
        var $block, $input, $url_label, $title_label, value_object, $link_submit, $vc_link_submit, $vc_link_nofollow,
          dialog;
    ...
    content: null, gutenbergParamObj: null, $frame: null, closeEditor: function (e) {
      e && e.preventDefault && e.preventDefault();
      var _this = this;
      _.delay(function () {
      ...
      $('[data-vc-action="open"]', _this.content).on('click', function (e) {
        ...
        iframeURL = window.wpbGutenbergEditorUrl ? window.wpbGutenbergEditorUrl : '/wp-admin/post-new.php?post_type=wpb_gutenberg_param', _this.gutenbergParamObj = new GutenbergParam({
          ...
          }, onError: _this.closeEditor, value: value,
  ...
  function renderGutenbergControls(iframe) {
    var $iframeDocument = $(iframe).contents(), postToolbar = $iframeDocument.find('.edit-post-header-toolbar'),
  ...
  var GutenbergParam = function (options) {
    return this.iframe = null, this.options = options || {}, this.value = this.options && this.options.value ? this.options.value : '', this.iframeLoaded = function () {
      ...
      (wpData = !!this.iframe.contentWindow.wp && this.iframe.contentWindow.wp.data) || (alertNotice = (localizations = !!window.i18nLocale && window.i18nLocale) && localizations.gutenbergDoesntWorkProperly ? localizations.gutenbergDoesntWorkProperly : 'Gutenberg plugin doesn\'t work properly. Please check Gutenberg plugin.', window.alert(alertNotice), this.options && this.options.onError && this.options.onError()), newPost = {
        ...
        guid: {raw: '/?', rendered: '/?'},
        title: {raw: ''},
        content: {raw: this.value, rendered: this.value},
      ...
      }), editor.setupEditor(newPost), renderGutenbergControls(this.iframe);
  ...
  vc.debug = !1, vc.map = _.isUndefined(window.vc_mapper) ? {} : window.vc_mapper, vc.roles = _.isUndefined(window.vc_roles) ? {} : window.vc_roles, vc.Storage = function () {
    ...
    url: window.ajaxurl, checksum: !1, locked: !1, create: function (model) {
      ...
      return _.isNull(matches) || _.each(matches, function (raw) {
        var shortcode, map_settings, sub_matches = raw.match(this.regexp(tags)), sub_content = sub_matches[5],
          sub_regexp = new RegExp('^[\\s]*\\[\\[?(' + _.keys(vc.map).join('|') + ')(?![\\w-])'), id = window.vc_guid(),
          atts_raw = window.wp.shortcode.attrs(sub_matches[3]), atts = {};
      ...
      var tags = _.keys(vc.map).join('|'), reg = this.regexp_split('vc_row'),
        starts_with_shortcode = new RegExp('^\\[(\\[?)(' + tags + ')', 'g'), _this = this, storage = {}, i = 0;
      content = wp.shortcode.replace('vc_section', content, function (data) {
        var toSave = {attrs: data.attrs.named, content: _this.wrapData(data.content)},
      ...
      return content = _.reduce(matches, function (mem, value) {
        ...
        return wp.shortcode.string({tag: 'vc_section', attrs: item.data.attrs, content: item.data.content});
      })), content;
    }, fetch: function () {
  ...
  vc.shortcode = Backbone.Model.extend({
    ...
    model: vc.shortcode, last_index: 0, getNextOrder: function () {
  ...
  vc.clone_index = 1, vc.shortcode_view = Backbone.View.extend({
    ...
    _loadDefaults: function () {
      var tag;
      tag = this.model.get('shortcode'), !!!vc.shortcodes.where({parent_id: this.model.get('id')}).length && !0 === this.use_default_content && _.isObject(vc.map[tag]) && _.isString(vc.map[tag].default_content) && vc.map[tag].default_content.length && (this.use_default_content = !1, vc.shortcodes.createFromString(vc.map[tag].default_content, this.model));
    },
      ...
      tag = model.get('shortcode'), params = model.get('params'), settings = vc.map[tag], _.defer(function () {
        ...
        if (name = param_settings.param_name, label_value = value = params[name], $admin_label = ($wrapper = this.$el.find('> .wpb_element_wrapper, > .vc_element-wrapper')).children('.admin_label_' + name), _.isObject(vc.atts[param_settings.type]) && _.isFunction(vc.atts[param_settings.type].render) && (value = vc.atts[param_settings.type].render.call(this, param_settings, value)), $wrapper.children('.' + param_settings.param_name).is('input,textarea,select')) $wrapper.children('[name=' + param_settings.param_name + ']').val(value); else if ($wrapper.children('.' + param_settings.param_name).is('iframe')) $wrapper.children('[name=' + param_settings.param_name + ']').attr('src', value); else if ($wrapper.children('.' + param_settings.param_name).is('img')) {
          ...
          $img = $wrapper.children('[name=' + param_settings.param_name + ']'), value && value.match(/^\d+$/) ? $.ajax({
            type: 'POST',
            url: window.ajaxurl,
            data: {action: 'wpb_single_image_src', content: value, size: 'thumbnail', _vcnonce: window.vcAdminNonce},
            dataType: 'html',
            context: this,
          }).done(function (url) {
            $img.attr('src', url);
  ...
  vc.saved_custom_css = !1, vc.createPreLoader = function () {
    ...
    appendShortcode: function (model) {
      ...
      view = this.getView(model), params = _.extend(vc.getDefaults(model.get('shortcode')), model.get('params')), model.set('params', params, {silent: !0}), parentModelView = !1 !== model.get('parent_id') && this.views[model.get('parent_id')], this.views[model.id] = view, model.get('parent_id') && this.views[model.get('parent_id')].unsetEmpty();
    ...
    addShortcode: function (model) {
      ...
      (params = _.extend(vc.getDefaults(model.get('shortcode')), model.get('params')), model.set('params', params, {silent: !0}), view = this.getView(model), parentModelView = !1 !== model.get('parent_id') && this.views[model.get('parent_id')], view.use_default_content = !0 !== model.get('cloned'), this.views[model.id] = view, parentModelView) ? (parentModelView.addShortcode(view), parentModelView.checkIsEmpty(), _this = this, _.defer(function () {
    ...
    addTextBlock: function (e) {
      ...
      return e && e.preventDefault && e.preventDefault(), row_params = {}, row = vc.shortcodes.create({
        shortcode: 'vc_row',
        params: row_params,
      }), column_params = {width: '1/1'}, column = vc.shortcodes.create({
        shortcode: 'vc_column',
        params: column_params,
        parent_id: row.id,
        root_id: row.id,
    ...
    createRow: function () {
      ...
      return row_params = {}, row = vc.shortcodes.create({
        shortcode: 'vc_row',
        params: row_params,
      }), column_params = {width: '1/1'}, vc.shortcodes.create({
        shortcode: 'vc_column',
        params: column_params,
        parent_id: row.id,
        root_id: row.id,
    ...
    updateElementsSorting: function (event, ui) {
      _.defer(function (app) {
        var parent = ui.item.parent().closest('[data-model-id]').data('model'), model = ui.item.data('model'),
          models = app.views[parent.id].$content.find('> [data-model-id]'), i = 0;
        if (!_.isNull(ui.sender)) {
          var old_parent_id = model.get('parent_id');
          vc.storage.lock(), model.save({parent_id: parent.id}), old_parent_id && app.views[old_parent_id].checkIsEmpty(), app.views[parent.id].checkIsEmpty();
        }
    ...
    updateRowsSorting: function (e, ui) {
      _.defer(function (app) {
        var parentNode = ui.item.parent(), $currentContainer = parentNode.closest('[data-model-id]'),
          newParentId = !!$currentContainer.length && $currentContainer.data('model').get('id'),
          model = ui.item.data('model'), tag = ui.item.data('element_type'),
        ...
        if (vc.check_relevance(parent_tag, tag) && parent_tag != tag) {
          ...
          parentNode.find(app.rowSortableSelector).each(function () {
            ...
            vc.storage.lock(), $(this).data('model').save({order: index});
          }), model.save({parent_id: newParentId}), oldParentId && app.views[oldParentId].checkIsEmpty(), newParentId && app.views[newParentId].checkIsEmpty();
    ...
    setSortable: function () {
      if (vc_user_access().partAccess('dragndrop')) return $('.wpb_main_sortable').sortable({
        ...
        out: function (event, ui) {
          ui.placeholder.removeClass('vc_hidden-placeholder'), ui.placeholder.css({maxWidth: ui.placeholder.parent().width()});
        },
    ...
    ($wpbVisualComposer = $('#wpb_visual_composer')).is('div') && (vc.app = new vc.visualComposerView({el: $wpbVisualComposer}), vc.accessPolicy.can('be_editor') && !vc_user_access().isBlockEditorIsEnabled() && vc.app.checkVcStatus(), vc.events.on('vc:backend_editor:show', function () {
  ...
  window.VcRowView = vc.shortcode_view.extend({
    ...
    convertRowColumns: function (layout) {
      var layout_split = layout.toString().split(/_/), columns = Shortcodes.where({parent_id: this.model.id}),
        new_columns = [], new_layout = [], new_width = '';
      return _.each(layout_split, function (value, i) {
        ...
        new_width = 3 < column_data.length ? column_data[0] + '' + column_data[1] + '/' + column_data[2] + column_data[3] : 2 < column_data.length ? column_data[0] + '/' + column_data[1] + column_data[2] : column_data[0] + '/' + column_data[1], new_layout.push(new_width), new_column_params = _.extend(_.isUndefined(columns[i]) ? {} : columns[i].get('params'), {width: new_width}), vc.storage.lock(), new_column = Shortcodes.create({
    ...
    validateCellsList: function (cells) {
      var b, return_cells = [], split = cells.replace(/\s/g, '').split('+');
    ...
    events: {
      ...
      ui.draggable.is('#wpb-add-new-element') ? vc.add_element_block_view({model: {position_to_add: 'end'}}).show(this) : ui.draggable.is('#wpb-add-new-row') && this.createRow();
    }, setEmpty: function () {
      ...
      return row_params = {}, column_params = {width: '1/1'}, row = Shortcodes.create({
        shortcode: 'vc_row_inner',
        params: row_params,
        parent_id: this.model.id,
      }), Shortcodes.create({shortcode: 'vc_column_inner', params: column_params, parent_id: row.id}), row;
    }, convertSize: function (width) {
      var numbers = width ? width.split('/') : [1, 1], range = _.range(1, 13),
      ...
      this.model.destroy(), parent_id && !vc.shortcodes.where({parent_id: parent_id}).length ? (parent = vc.shortcodes.get(parent_id), _.contains(['vc_column', 'vc_column_inner'], parent.get('shortcode')) || parent.destroy()) : parent_id && (parent = vc.shortcodes.get(parent_id)) && parent.view && parent.view.setActiveLayoutButton && parent.view.setActiveLayoutButton();
    }, remove: function () {
    ...
    designHelpersSelector: '> .vc_controls-row .vc_column-edit', setColumnClasses: function () {
      var disable = this.model.getParam('disable_element'),
        disableClass = 'vc_hidden-xs vc_hidden-sm  vc_hidden-md vc_hidden-lg';
      this.disable_element_class && this.$el.removeClass(this.disable_element_class), _.isEmpty(disable) || (this.$el.addClass(disableClass), this.disable_element_class = disableClass);
    }, buildDesignHelpers: function () {
      ...
      if (vc_user_access().partAccess('dragndrop')) {
        ...
        this.$content.sortable({
          ...
          update: function (event, ui) {
            ...
            $elements.each(function () {
              ...
              if (model.set('order', index), $elements.length - 1 > index && vc.storage.lock(), !_.isNull(ui.sender)) {
                ...
                vc.storage.lock(), model.save({parent_id: parent.id}), old_parent_id && vc.app.views[old_parent_id].checkIsEmpty(), vc.app.views[parent.id].checkIsEmpty();
              }
              model.save();
            });
          },
          ...
          out: function (event, ui) {
            ui.placeholder.removeClass('vc_hidden-placeholder'), ui.placeholder.css({maxWidth: ui.placeholder.parent().width()}), _.isNull(ui.sender) || !ui.sender.length || ui.sender.find('> [data-element_type]:not(.ui-sortable-helper):visible').length || ui.sender.addClass('vc_sorting-empty-container');
          },
        });
      }
    },
      ...
      return window.VcAccordionView.__super__.render.call(this), vc_user_access().shortcodeAll('vc_accordion_tab') ? vc_user_access().partAccess('dragndrop') && this.$content.sortable({
        ...
        stop: function (event, ui) {
          ui.item.prev().triggerHandler('focusout'), $(this).find('> .wpb_sortable').each(function () {
            $(this).data('model').save({order: $(this).index()});
          });
        },
    ...
    addTab: function (e) {
      ...
      var tab_title = window.i18nLocale.tab, tabs_count = this.$tabs.find('[data-element_type=vc_tab]').length,
      ...
      if (this.model.get('shortcode') === 'testimonial_slider') {
        ...
        tab_title = 'Testimonial';
        return vc.shortcodes.create({
          shortcode: 'testimonial',
          params: {title: tab_title, id: tab_id},
          parent_id: this.model.id,
        });
      }
      else if(this.model.get('shortcode') === 'sar_carousel') {
        tab_title = 'Item';
        return vc.shortcodes.create({
          shortcode: 'sar_carousel_item',
          params: {title: tab_title, id: tab_id},
          parent_id: this.model.id
        });
      }
    ...
    events: {
      ...
      if (this.model.destroy(), !vc.shortcodes.where({parent_id: parent_id}).length) return vc.shortcodes.get(parent_id).destroy(), !1;
      var params = this.model.get('params'),
    ...
    updateSorting: function (event, ui) {
      ...
      (self = this).$sortable.find(this.sortableSelector).each(function () {
        ...
        modelId = ($this = $(this)).attr(self.sortableUpdateModelIdSelector), shortcode = vc.shortcodes.get(modelId), vc.storage.lock(), shortcode.save({order: self.getIndex($this)});
  ...
  'use strict';
  var attachmentCompatRender, attachCb = [], media = wp.media, origFeaturedImageSet = media.featuredImage.set,
    origEditorSendAttachment = media.editor.send.attachment, l10n = i18nLocale, workflows = {};
  ...
  attachmentCompatRender = _.extend(media.view.AttachmentCompat.prototype.render), media.view.AttachmentCompat.prototype.render = function () {
    ...
    $vc_editor_element: null, getData: function () {
      ...
      var $block = media.vc_editor.$vc_editor_element.closest('.edit_form_line'),
        ...
        $img_ul = $block.find('.gallery_widget_attached_images_list'), $thumbnails_string = '',
  ...
  (vcPointersController = function (Pointer, texts) {
    ...
    init: function () {
      ...
      $.post(window.ajaxurl, {pointer: this.pointerId, action: 'dismiss-wp-pointer'}), this.messagesDismissed = !0;
    },
  ...
  undo_redo_core = {
    stack: [], stackPosition: 0, stackHash: JSON.stringify(''), zeroState: null, locked: !1, add: function (data) {
  ...
  $(function () {
    ...
    window.vc && window.vc.events && ($undoControl = $('#vc_navbar-undo'), $redoControl = $('#vc_navbar-redo'), checkControls = function () {
      ...
      vc.storage.setContent(content), vc.shortcodes.fetch({reset: !0}), _.delay(function () {
      ...
      if ($(this).is('[disabled]') || window.vc.undoRedoApi.isLocked()) e && e.preventDefault && e.preventDefault(); else {
